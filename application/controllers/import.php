...
// create account for that member
    ...
    function import_mortuary() {
        $this->data['title'] = 'Import Mortuary';
            ...
            if ($number_sheet == 1) {
                ...
                foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                    ...
                    for ($i = 2; $i <= $highestRow; $i++) {
                        ...
                        if ($error_number == 0) {
                            ...
                            if ($continue) {
                                ...
                                $receipt = $this->mortuary_model->contribution_transaction($trans_type, $pid, $member_id, $amount, $paymethod, $comment, $check_number_received, $month = '', $auto = 0, $date);
                                if ($receipt) {
                                    $imported ++;
                                } else {
                                    $total_error++;
                    ...
                    if ($total_error == 0) {
                        unlink("./uploads/$file_name");
                        $this->session->set_flashdata('message', $imported . ' row(s) migrated successfully !!');
                        redirect('import/import_mortuary', 'refresh');
                    } else {
                        unlink("./uploads/$file_name");
                        $this->data['title'] = "Migrate Feedback";
                        $this->data['feedback'] = $report_array;
        ...
        $this->data['content'] = 'migration/import_mortuary';
        $this->load->view('template', $this->data);
    }
