...
<head>
    ...
    <?php
    /*
     * TCPDF FALLBACK ONLY - the on-screen sheet's look is not the goal here.
     *
     * The Print tab and the exported PDF now render
     * cashiering/print_cash_count_sheet.php, a standalone Tailwind page that mirrors
     * the sheet the cashier sees. This file survives for the hosts where no
     * Chromium/Edge/wkhtmltopdf binary can be launched, so
     * Cashiering::_cashiering_pdf_renderer() returns FALSE. TCPDF cannot parse a full
     * HTML document, a stylesheet or a Tailwind class, so it is handed the <body>
     * markup only (see Cashiering::_html_body()).
     *
     ...
     *
     * When the sheet changes, change the Tailwind view first. This one only has to
     * stay complete and legible, not identical - the two are deliberately allowed to
     * look different, because the renderer that draws this one cannot draw that one.
     */
    ...
    /*
     * Official cooperative crest, the same one the loan forms print.
     * loan_form_logo_src() returns an inline data URI when it can read the file,
     * which is what lets both renderers show the crest: the headless browser
     * loads the page from a file:// temp path (no HTTP fetch), and TCPDF only
     * ever accepts a local path or an embedded data URI.
     */
    $cc_logo_file = (defined('TAPSTEMCO_FORM_LOGO') && TAPSTEMCO_FORM_LOGO !== '')
        ? TAPSTEMCO_FORM_LOGO
        : ((!empty($company->logo)) ? $company->logo : '');
    $cc_logo_src = function_exists('loan_form_logo_src') ? loan_form_logo_src($cc_logo_file) : '';
...
<body>
    ...
    <table style="width:100%;">
        <tr>
            <td style="text-align:center; border-bottom:1px solid #000; padding-bottom:4px;">
                <?php if ($cc_logo_src !== '') { ?>
                    <img src="<?php echo $cc_logo_src; ?>" width="44" height="44" alt="logo"/>
                <?php } ?>
    ...
    <table style="width:100%;">
        ...
        <tr>
            ...
            <td style="font-size:9pt; text-align:right;"><?php echo !empty($report->id) ? 'Report #' . (int) $report->id : 'DRAFT - not yet submitted'; ?></td>
        </tr>
    </table>
    <br/>
