...
<?php
$chart_sub_types = isset($chart_sub_types) ? $chart_sub_types : array();
$row_count = is_array($chart_sub_types) ? count($chart_sub_types) : 0;
?>
    ...
    <div class="member-table-panel">
        <div class="panel-head">
            <div class="panel-head-left">
                ...
                <h4><?php echo lang('chart_sub_type_list'); ?></h4>
            </div>
            <div class="head-actions">
                ...
                <a class="btn btn-primary btn-sm" href="<?php echo site_url(current_lang() . '/finance/chart_sub_type_create'); ?>">
                    <i class="fa fa-plus"></i> <?php echo lang('chart_sub_type_create'); ?>
                </a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-striped table-bordered member-table">
                ...
                <tbody>
                    <?php if ($row_count > 0) {
                        $i = 1;
                        foreach ($chart_sub_types as $value) {
                            $chart_type = $this->finance_model->account_type(null, $value->accounttype)->row();
                            ?>
                            <tr>
                                <td style="text-align:center;"><?php echo $i++; ?></td>
                                <td><?php echo $chart_type ? htmlspecialchars($chart_type->name, ENT_QUOTES, 'UTF-8') : '-'; ?></td>
                                <td><span class="member-id-chip"><?php echo htmlspecialchars($value->sub_account, ENT_QUOTES, 'UTF-8'); ?></span></td>
                                <td><?php echo htmlspecialchars($value->name, ENT_QUOTES, 'UTF-8'); ?></td>
                                <td>
                                    <div class="action-btns">
                                        <a class="btn btn-primary btn-xs" href="<?php echo site_url(current_lang() . '/finance/chart_sub_type_edit/' . encode_id($value->id)); ?>">
                                            <i class="fa fa-edit"></i> <?php echo lang('button_edit'); ?>
                                        </a>
                                        <a href="javascript:void(0);" class="btn btn-danger btn-xs btn-delete-chart-sub-type" data-id="<?php echo encode_id($value->id); ?>" data-name="<?php echo htmlspecialchars($value->name, ENT_QUOTES, 'UTF-8'); ?>">
