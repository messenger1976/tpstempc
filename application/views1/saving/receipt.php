...
<div id="receipt">
    ...
    <table id="receipt_after_title">
       <?php //$deposited_to = client_user_info($trans->FROM_TO_PIN, TRUE); ?>
        <tr>
            <td>Date/Tarehe <br/> <b> <?php $ex = explode(' ', $trans->trans_date); echo format_date($ex[0], FALSE) ; ?></b></td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            <td>Account Number <br/>Namba ya Akaunti<br/><b> <?php echo $trans->account ; ?></b></td>
            <td>Account Holder's Name<br/>Jina la Mwenye Akaunti<br/><b>   <?php
            $account_name = $this->finance_model->saving_account_name($trans->account);
          echo $account_name;  ?></b></td>
        </tr>
        <tr>
            <td>Deposit/Withdrawal<br/>Kuweka/Kutoa<br/><b> <?php echo ($trans->trans_type == 'CR' ? lang('CR'): lang('DR')) ; ?></b></td>
        ...
        <tr>
            <td>Customer/Mteja <br/>
                <b><?php echo ($trans->customer_name != '' ? $trans->customer_name.'<br/>':''); ?></b>
                <br/>.....................................</td>
