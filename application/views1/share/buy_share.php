...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#pid").autocomplete("<?php echo site_url(current_lang() . '/saving/autosuggest/pid'); ?>",
        {
            ...
            serverURLq:'<?php echo site_url(current_lang() . '/saving/search_member_share/'); ?>',
            secondID: 'member_id',
            ...
            matchContains:true,
            column: 'PID',
             balance_share : '<?php echo lang('balance_share'); ?>',
            total_shareamount : '<?php echo lang('total_shareamount'); ?>',
            min_share : '<?php echo lang('min_share'); ?>',
            max_share : '<?php echo lang('max_share'); ?>',
            remainshare : '<?php echo lang('remainshare'); ?>'
        }); 
        ...
        if(pid.length > 0){
            ...
            $.ajax({
                url:'<?php echo site_url(current_lang() . '/saving/search_member_share/'); ?>',
                type:'POST',
                data:{
                    value:pid,
                    column :'PID'
                ...
                success: function(data){
                    ...
                    if(json['success'].toString() == 'N'){
                        ...
                        output += '<br/><br/><div style="border-bottom:1px dashed #ccc;"><strong><?php echo lang('balance_share'); ?> : </strong> '+share["share"]+'</div>';
                        output += '<div style="border-bottom:1px dashed #ccc;"><strong><?php echo lang('total_shareamount'); ?> : </strong> '+addCommas(share["amount"])+'</div>';
                        output += '<div style="border-bottom:1px dashed #ccc;"><strong><?php echo lang('min_share'); ?> : </strong> '+share["min_share"]+'</div>';
                        output += '<div style="border-bottom:1px dashed #ccc;"><strong><?php echo lang('max_share'); ?> : </strong> '+share["max_share"]+'</div>';
                        output += '<div style="border-bottom:1px dashed #ccc;"><strong><?php echo lang('remainshare'); ?> : </strong> '+(parseInt(share["max_share"])- parseInt(share["share"]))+'</div>';
