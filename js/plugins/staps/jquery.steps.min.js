        ...
        if (((0 > d || d >= c.stepCount) && R(Y), !(b.forceMoveForward && d < c.currentIndex))) {
            ...
            return (
                a.triggerHandler("stepChanging", [c.currentIndex, d])
                    ? ((c.currentIndex = d), O(a, b, c), E(a, b, c, e), D(a, b, c), A(a, b, c), P(a, b, c, d, e), a.triggerHandler("stepChanged", [d, e]))
                ...
                switch (r($, e.contentMode)) {
                    case $.iframe:
                        b.find(".content > .body")
                            .eq(d.currentIndex)
                            .empty()
                            .html('<iframe src="' + e.contentUrl + '" frameborder="0" scrolling="no" />')
                            .data("loaded", "1");
                        break;
                    case $.async:
    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
                ...
                f = "";
            b.forceMoveForward || b.enableCancelButton && (f += e.format("cancel", b.labels.cancel)),
