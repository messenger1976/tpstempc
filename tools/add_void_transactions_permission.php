<?php
/**
 * Add Void_transactions permission to Module 6 (Finance)
 * Run once: http://your-domain/tools/add_void_transactions_permission.php
 * Or use sql/add_void_transactions_permission.sql in phpMyAdmin.
...
$module_id = 6;
$permission_name = 'Void_transactions';

echo "<!DOCTYPE html><html><head><title>Add Void_transactions</title>
<style>
...
echo "<h1>Adding Void Transactions Permission</h1>";
...
while ($group = $groups->fetch_assoc()) {
    ...
    $has_review = $mysqli->query("SELECT id FROM `access_level` WHERE group_id = {$gid} AND Module = {$module_id} AND link = 'Review_journal_entry' AND allow = 1")->fetch_assoc();
    $allow = ($has_review || $gid === 1) ? 1 : 0;
    ...
    if (!$check) {
        ...
        echo "<p class='success'>Enabled for group (had Review_journal_entry): " . htmlspecialchars($group['name']) . "</p>";
    } else {
...
echo "<p>In <strong>Auth → Group Role</strong> under Finance, assign <strong>Void_transactions</strong> separately from <strong>Review_journal_entry</strong>.</p>";
echo "<p>Menu: <strong>Finance → Void Transactions</strong> → <code>/finance/void_transactions</code></p>";
